...
import { getReminders, postReminder, deleteReminder, updateReminder, getDefaultColor, updateDefaultColor } from './gateWay.js';
...
class Calendar extends Component {
    ...
    handleCreate = (e) => {
        ...
        if (className === "header__create") {
            ...
            newFormData.date = moment(+parentElement.dataset.date).format("YYYY-MM-DD");
    ...
    handleOpenReminder = (e) => {
        e.stopPropagation();
        if (e.target.id) {
            const reminderSelected = this.state.reminders.find(reminder => reminder.id === e.target.id);
            this.setState({
    ...
    render() {
        const { currentWeekStart, reminders,
            isPopUpOpen, reminderForm,
            isLoading, defaultColor,
            isError } = this.state;
        return (
            <>
                {isPopUpOpen &&
